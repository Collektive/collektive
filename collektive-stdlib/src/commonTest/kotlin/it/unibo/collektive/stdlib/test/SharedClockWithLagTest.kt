...
class SharedClockWithLagTest {
    ...
    fun gridWithExecutionFrequency(size: Int, frequency: Int) = mooreGrid<Instant>(size, size, { _, _ ->
        ...
        sharedClockWithLag(times[localId]).also { increaseTime(localId, frequency) }
    }
    ...
    fun `After a single round of computation all devices should return DISTANT_PAST`() {
        ...
        environment.status().values.size shouldBe 4
        environment.nodes.forEach { node ->
            environment.shouldBeInstant(node.id, DISTANT_PAST + node.id.seconds)
        }
    }
    ...
    fun `In two subsequent rounds of computation the devices should increase their time by their delta seconds`() {
        ...
        environment.cycleInOrder()
        environment.nodes.forEach { node ->
            environment.shouldBeInstant(node.id, DISTANT_PAST + node.id.seconds + environment.nodes.size.seconds)
        }
    }
