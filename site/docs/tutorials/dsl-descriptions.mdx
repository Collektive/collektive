...
\`minBy(base: T, crossinline selector: (T) -> R)\` is a field operator: returns the element yielding the smallest value of the given \`selector\`.
...
\`replaceMatching(replaceWith: T, crossinline predicate: (T) -> Boolean)\` is a field operations: returns a new field containing \`replaceWith\` for each element that satisfies the \`predicate\`.
` },
{ name:`excludeSelf(): 
