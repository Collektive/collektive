...
/**
 * Sixteen bits to shift.
 */
const val BIT_16 = 16

/**
 * Twenty-four bits to shift.
 */
const val BIT_24 = 24

/**
 * Thirty-two bits to shift.
 */
const val BIT_32 = 32
...
/**
 * Forty-eight bits to shift.
 */
const val BIT_48 = 48

/**
 * Fifty-six bits to shift.
 */
const val BIT_56 = 56

/**
 * A summary of a path based on the Murmur3 hash function.
 */
class Murmur3Path(path: Path) : PathSummary {
    ...
    private fun Any?.toBytes(): ByteArray = when (this) {
        is Iterable<*> -> {
            ...
            ByteArray(size).also { destination ->
                var index = 0
                arrays.forEach { bytes ->
                    bytes.copyInto(destination, destinationOffset = index)
                    index += bytes.size
                }
            }
        }
        is Byte -> byteArrayOf(this)
        is UByte -> toShort().toBytes()
        is Short -> toInt().toBytes()
        is UShort -> toInt().toBytes()
        is Int -> listOf(BIT_24, BIT_16, BIT_8, NO_SHIFT)
            .map { (this shr it).toByte() }
            .toByteArray()
        is UInt -> toLong().toBytes()
        is Long -> listOf(BIT_56, BIT_48, BIT_40, BIT_32, BIT_24, BIT_16, BIT_8, NO_SHIFT)
            .map { (this shr it).toByte() }
            .toByteArray()
        is ULong -> listOf(BIT_56, BIT_48, BIT_40, BIT_32, BIT_24, BIT_16, BIT_8, NO_SHIFT)
            .map { (this shr it).toByte() }
            .toByteArray()
        ...
        is ByteArray -> this
        is Map<*, *> -> toList().toBytes()
        null -> byteArrayOf(0)
        is Array<*> -> toList().toBytes()
        else -> error("Unsupported type: ${this::class.simpleName}")
    }

    /**
     * Converts a path into a byte array and then hashes it using the 128-bit version of the Murmur3 hash function.
     */
    val hash = MurmurHash3().hash128x64(path.tokens().toBytes())

    override fun equals(other: Any?): Boolean = when {
        this === other -> true
        other !is Murmur3Path -> false
        else -> hash.contentEquals(other.hash)
    }

    override fun hashCode(): Int = hash[0].toInt()

    @OptIn(ExperimentalStdlibApi::class)
    override fun toString(): String = hash.toBytes().toHexString()
}
...
public class MurmurHash3(private val seed: UInt = 0u) {
    /**
     * Hashes the given [key] using the 32-bit version of the Murmur3 hash function.
     */
    public fun hash32x86(key: ByteArray): UInt {
        ...
        var k = 0u
        if (rem == 3) {
            k = k xor (key.getUInt(index + 2) shl BIT_16)
        }
        if (rem >= 2) {
            k = k xor (key.getUInt(index + 1) shl BIT_8)
        }
        if (rem >= 1) {
    ...
    /**
     * Hashes the given [key] using the 128-bit version of the Murmur3 hash function.
     */
    public fun hash128x86(key: ByteArray): Array<UInt> {
        ...
        var h4 = seed
        val len = key.size
        val nblocks = len / BIT_16

        for (i in 0 until nblocks * BIT_16 step BIT_16) {
            val k1 = key.getLittleEndianUInt(i)
            val k2 = key.getLittleEndianUInt(i + 4)
            val k3 = key.getLittleEndianUInt(i + BIT_8)
        ...
        var k4 = 0u
        if (rem == 15) {
            k4 = k4 xor (key.getUInt(index + 14) shl BIT_16)
        }
        if (rem >= 14) {
            k4 = k4 xor (key.getUInt(index + 13) shl BIT_8)
        }
        if (rem >= 13) {
        ...
        if (rem >= 12) {
            k3 = k3 xor (key.getUInt(index + 11) shl BIT_24)
        }
        if (rem >= 11) {
            k3 = k3 xor (key.getUInt(index + 10) shl BIT_16)
        }
        if (rem >= 10) {
            k3 = k3 xor (key.getUInt(index + 9) shl BIT_8)
        }
        if (rem >= 9) {
            k3 = k3 xor key.getUInt(index + BIT_8)
            h3 = h3 xor k3.mix(R3_128x86, C3_128x86, C4_128x86)
        }
        if (rem >= 8) {
            k2 = k2 xor (key.getUInt(index + 7) shl BIT_24)
        }
        if (rem >= 7) {
            k2 = k2 xor (key.getUInt(index + 6) shl BIT_16)
        }
        if (rem >= 6) {
            k2 = k2 xor (key.getUInt(index + 5) shl BIT_8)
        }
        if (rem >= 5) {
        ...
        if (rem >= 4) {
            k1 = k1 xor (key.getUInt(index + 3) shl BIT_24)
        }
        if (rem >= 3) {
            k1 = k1 xor (key.getUInt(index + 2) shl BIT_16)
        }
        if (rem >= 2) {
            k1 = k1 xor (key.getUInt(index + 1) shl BIT_8)
        }
        if (rem >= 1) {
    ...
    public fun hash128x64(key: ByteArray): Array<ULong> {
        ...
        for (i in 0 until nblocks * BIT_16 step BIT_16) {
            ...
            val k2 = key.getLittleEndianLong(i + BIT_8)
        ...
        var k2 = 0uL
        if (rem == 15) {
            k2 = k2 xor (key.getULong(index + 14) shl BIT_48)
        }
        if (rem >= 14) {
            k2 = k2 xor (key.getULong(index + 13) shl BIT_40)
        }
        if (rem >= 13) {
            k2 = k2 xor (key.getULong(index + 12) shl BIT_32)
        }
        if (rem >= 12) {
            k2 = k2 xor (key.getULong(index + 11) shl BIT_24)
        }
        if (rem >= 11) {
            k2 = k2 xor (key.getULong(index + 10) shl BIT_16)
        }
        if (rem >= 10) {
            k2 = k2 xor (key.getULong(index + 9) shl BIT_8)
        }
        if (rem >= 9) {
            k2 = k2 xor key.getULong(index + BIT_8)
            h2 = h2 xor k2.mix(R3_128x64, C2_128x64, C1_128x64)
        }
        if (rem >= 8) {
            k1 = k1 xor (key.getULong(index + 7) shl BIT_56)
        }
        if (rem >= 7) {
            k1 = k1 xor (key.getULong(index + 6) shl BIT_48)
        }
        if (rem >= 6) {
            k1 = k1 xor (key.getULong(index + 5) shl BIT_40)
        }
        if (rem >= 5) {
            k1 = k1 xor (key.getULong(index + 4) shl BIT_32)
        }
        if (rem >= 4) {
            k1 = k1 xor (key.getULong(index + 3) shl BIT_24)
        }
        if (rem >= 3) {
            k1 = k1 xor (key.getULong(index + 2) shl BIT_16)
        }
        if (rem >= 2) {
            k1 = k1 xor (key.getULong(index + 1) shl BIT_8)
        }
        if (rem >= 1) {
    ...
    private fun ByteArray.getLittleEndianUInt(index: Int): UInt {
        return this.getUInt(index) or
            (this.getUInt(index + 1) shl BIT_8) or
            (this.getUInt(index + 2) shl BIT_16) or
            (this.getUInt(index + 3) shl BIT_24)
    }

    private fun ByteArray.getLittleEndianLong(index: Int): ULong {
        return this.getULong(index) or
            (this.getULong(index + 1) shl BIT_8) or
            (this.getULong(index + 2) shl BIT_16) or
            (this.getULong(index + 3) shl BIT_24) or
            (this.getULong(index + 4) shl BIT_32) or
            (this.getULong(index + 5) shl BIT_40) or
            (this.getULong(index + 6) shl BIT_48) or
            (this.getULong(index + 7) shl BIT_56)
    }
